...
# 1. Extracts year and month from the date column and create two another columns
#def extract_year_month(dataframe):

#    year = dataframe['dteday'].apply(lambda x: int(x.split('-')[0]))
#    month = dataframe['dteday'].apply(lambda x: int(x.split('-')[1]))
...
#    data_frame['month']= extract_year_month(data_frame)[1]
    ...
    required_columns = config.model_config_.features
    for column in required_columns:
        if column not in data_frame.columns:
            data_frame[column] = None  # or some default value
